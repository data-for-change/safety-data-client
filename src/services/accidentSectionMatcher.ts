...
function pointInExpandedBounds(
  ...
  return (
    p.lat >= bounds.minLat - dLat &&
    p.lat <= bounds.maxLat + dLat &&
    p.lng >= bounds.minLng - dLng &&
    p.lng <= bounds.maxLng + dLng
  );
}
...
export function mapAccidentsToStreetSections(
  ...
  for (const a of accidents) {
    ...
    let shortlist = candidates.filter(s => pointInExpandedBounds(p, s.bounds, opt.boundsPaddingMeters));

    if (shortlist.length === 0 && opt.enableGeoOnlyFallback) {
